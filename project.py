...
import httplib2
import json
from flask import make_response, redirect, url_for, jsonify, flash
import requests
...
def editCategory(category_id):
    ...
    if editedCategory.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not \
               authorized to edit this category. Please create your own category in order to edit.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
...
def newItem(category_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized \
               to add items to this category.');}</script> \
...
# Show descriptions of particular item under a category
...
def editItem(category_id, item_id):
    ...
    if editedItem.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized \
               to edit items under this category.');}</script>\
               <body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
...
def deleteItem(category_id, item_id):
    ...
    if itemToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized \
               to delete items under this category.');}</script> \
               <body onload='myFunction()''>"
    if request.method == 'POST':
